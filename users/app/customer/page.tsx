...
async function getFeaturedProducts() {
  try {
    ...
    return rows.map((item) => ({
      ...
      danhmuc_id: Number(item.danhmuc_id || 0),
    }));
  } catch (error) {
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {CATEGORIES.map((category) => {
            ...
            return (
              <Link
                key={category.id}
                href={`/customer/products?danhmuc_id=${category.id}`}
              ...
              >
                <div
                  className={`h-40 bg-gradient-to-br ${category.color} flex items-center justify-center`}
                >
                  <Icon
